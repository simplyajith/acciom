...
def get_db_details(db_type, hostname, db_name, username, password, table_name,
    ...
    try:
        ...
        raise Exception(
            "Exception Occurred in get_db_details method. Exception: {}".format(
                e))
...
def get_count(db_detail):
    try:
        ...
        raise Exception(
            'Exception Occurred in get_count() method. Exception: {}'.format(
                e))
...
def get_df_select(db_detail, start, end):
    try:
        ...
        if not db_detail["custom_query"]:
            if db_detail["db_type"] == "sqlserver":
                ...
                query = "(SELECT * FROM {} " \
                        "ORDER BY 1 OFFSET {} " \
                        "ROWS FETCH NEXT  {} " \
                        "ROWS ONLY ) t ".format(db_detail["table_name"], start,
                                                end)
            ...
            if db_detail["db_type"] == "sqlserver":
                ...
                query = "(SELECT * FROM ({}) tt " \
                        "ORDER BY 1 OFFSET {} " \
                        "ROWS FETCH NEXT  {} " \
                        "ROWS ONLY ) t ".format(db_detail["custom_query"],
                                                start, end)
        df = sqlContext.read.format("jdbc").option(
            "url", url).option("driver", driver).option(
            "dbtable", query).option("user", db_detail["username"]).option(
        ...
        raise Exception(
            "Exception Occurred in get_df_select method. Exception: {}".format(
                e))
...
def run(src_db, dest_db, src_start, src_end, tgt_start, tgt_end):
    try:
        ...
        raise Exception(
            "Exception Occurred in run method. Exception: {}".format(e))
...
def data_validation(df_src, df_dest):
    try:
        ...
        raise Exception(
            "Exception Occurred in data_validation method. Exception:{}".format(
                e))
...
def handle_data_validation_error(exception):
    requests.post(testcases_tbr['datavalidation'],
                  json={'result': 'error', 'exception': str(exception)})
...
if __name__ == '__main__':
    try:
        ...
        testcases_tbr = dict(
            [arg.split('=', maxsplit=1) for arg in sys.argv[18:]])
        ...
        for testcase, api_end_point in testcases_tbr.items():
            ...
            if testcase == "datavalidation":
                ...
                result["dest_to_src"] = dest_to_src[
                                        :target_record_count] if isinstance(
                    dest_to_src, list) else []
